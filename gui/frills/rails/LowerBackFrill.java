/* Name: LowerBackFrill.java
 *
 * What:
 *   This file defines a class for drawing Diagonal tracks on the Screen.
 *   The tracks run from the Left edge to the Bottom edge.
 */
...
public class LowerBackFrill
    extends RailFrill {

  /**
   * constructs the LowerBackfrill.
   */
  public LowerBackFrill() {
    super(Sides.BOTTOM, Sides.LEFT,
  ...
  public void setDOT(int dot) {
    super.setDOT(dot);
    if ( (Dot == Sides.BOTTOM) || (Dot == Sides.LEFT)) {
      buildArrowHead(Dot);
    }
  }
...
//    int w = HIT_WIDTH;
    ...
    if (Gaps[Sides.LEFT]) {
      MyClip.y = yedge(Sides.LEFT, bounds) + PointGap;
      MyClip.height = clip.height - (MyClip.y - bounds.y);
    }
    if (Gaps[Sides.BOTTOM]) {
      MyClip.width = (bounds.width / 2) - PointGap - (MyClip.x - clip.x);
    }
    if (Dot == Sides.BOTTOM) {
      buildArrowHead(Dot);
    }
...
//    ypoints[0] = bounds.y + (bounds.width / 2) - w;
//    ypoints[1] = bounds.y + (bounds.width / 2) + w;
